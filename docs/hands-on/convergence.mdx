...
cat <<EOF> fit.gnu
#!/usr/bin/env gnuplot 
set terminal pngcairo enhanced   font  'Helvetica,18' size 800,900
set output "eneVsalat.png"
set border lc "black" lw 2.5
set tics scale 2.5
set format y "%.4f"
set title "Si "
set xlabel "Lattice parameter a (a.u)"
set ylabel "Energy (Ry)"
set fit quiet
mur(x)=Emin+1/14703.6*(B*x**3/Bp*(1*(a0**3/x**3)**Bp/(Bp-1)+1)-B*a0**3/(Bp-1))

Emin=-15.83
a0=10.2
B=1000
Bp=4
   file = '../src/silicon/etot-vs-alat.dat'
   fit mur(x) file via Emin,a0,B,Bp
  plot '../src/silicon/etot-vs-alat.dat' lw 3,  mur(x)  w l lw 3
